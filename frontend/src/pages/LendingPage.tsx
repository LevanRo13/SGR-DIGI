...
export default function LendingPage() {
  const [walletAura, setWalletAura] = useState(5000);
  ...
  // Constantes (Componente 2: Oráculo Mock)
  const AURA_PRICE = 1.0; // $1
  const XLM_PRICE = 0.10; // $0.10
  const C_FACTOR = 0.70; // 70% factor de colateral
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900">
            Préstamos Protocolo BLEND
          </h1>
        </div>
        <p className="text-slate-500 max-w-2xl text-lg">
          Usá tus garantías tokenizadas AURA como colateral para pedir prestado XLM.
        </p>
      </div>
      ...
      <section className="grid md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-sm font-medium text-slate-500 mb-1">Colateral Total (AURA)</p>
        ...
        <div className="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-sm font-medium text-slate-500 mb-1">Deuda Total (XLM)</p>
            <div className="text-3xl font-bold text-slate-900 flex items-baseline gap-2">
        ...
        <div className="bg-white rounded-[24px] border border-slate-200 p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-1">
              <p className="text-sm font-medium text-slate-500">Factor de Salud</p>
            ...
            <p className="text-xs text-slate-500 mt-1">
              &lt; 1.0 significa que tu colateral puede ser liquidado.
            </p>
          </div>
          
          <div className="mt-4 bg-slate-50 rounded-xl p-3 border border-slate-100">
             <div className="flex justify-between text-xs mb-1">
                <span className="text-slate-500">Límite de préstamo usado</span>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <section className="bg-white rounded-[28px] border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold tracking-tight text-slate-900">Depositar Colateral</h2>
              <p className="text-sm text-slate-500">Depositá AURA para habilitar préstamos</p>
            </div>
          ...
          <form onSubmit={handleSupply} className="space-y-6">
            <div className="relative">
              <label htmlFor="supply" className="block text-sm font-medium text-slate-700 mb-2">Cantidad a depositar</label>
            ...
            {simulateSupply > 0 && (
               <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Nuevo saldo depositado</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Nuevo límite de préstamo</span>
                    <div className="flex items-center gap-2 font-medium">
        ...
        <section className="bg-white rounded-[28px] border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold tracking-tight text-slate-900">Pedir prestado XLM</h2>
              <p className="text-sm text-slate-500">Pedí prestado hasta el 70% del valor del colateral</p>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-xs text-slate-500 mb-0.5">Disponible para préstamo</span>
          ...
          <form onSubmit={handleBorrow} className="space-y-6">
            <div className="relative">
              <label htmlFor="borrow" className="block text-sm font-medium text-slate-700 mb-2">Cantidad a pedir prestado</label>
              <div className="relative flex items-center">
                <input
                  id="borrow"
                  type="number"
            ...
            {simulateBorrow > 0 && (
               <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Nuevo Factor de Salud</span>
                    <div className="flex items-center gap-2 font-medium">
                  ...
                  {simulatedHealthFactor < 1.05 && (
                    <div className="text-xs text-red-600 bg-red-50 p-2 rounded-lg flex items-center gap-1.5 mt-2">
                      ...
                      Advertencia: Alto riesgo de liquidación
                    </div>
                  )}
               </div>
            )}
